  ...
  return (
    <Stack spacing={3}>
      <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" spacing={2}>
        ...
        <Button startIcon={<AddIcon />} variant="contained" onClick={() => { setEditingItem(null); setFormOpen(true); }}>
          Novo
        </Button>
      </Stack>

      <Paper sx={{ border: '1px solid #E5E7EB', boxShadow: 'none', overflow: 'hidden' }}>
        <Table>
          <TableHead>
            <TableRow>
              ...
              <TableCell>{isIncomePage ? 'Dia combinado para receber' : 'Vencimento e pagamento'}</TableCell>
          ...
          <TableBody>
            {items.map((item) => (
              <TableRow key={item.id}>
                ...
                <TableCell>
                  <Typography variant="caption" color="text.secondary" display="block">
                    {isIncomePage ? 'Dia combinado para receber' : `Situacao: ${item.status}`}
                  </Typography>
                  {isIncomePage ? formatDate(item.dueDate) : `Venc.: ${formatDate(item.dueDate)}${item.paymentDate ? ` / Pago: ${formatDate(item.paymentDate)}` : ''}`}
                </TableCell>
                <TableCell align="right">{formatMoney(item.amount)}</TableCell>
                <TableCell align="right">
                  <Tooltip title="Editar">
                    <IconButton onClick={() => { setEditingItem(item); setFormOpen(true); }}><EditIcon /></IconButton>
                  </Tooltip>
                  <Tooltip title="Excluir">
                    <IconButton color="error" onClick={() => removeItem(item)}><DeleteIcon /></IconButton>
                  </Tooltip>
                </TableCell>
              </TableRow>
            ))}
      ...
      <FinancialItemForm
        ...
        onSubmit={async (data) => {
          ...
          if (editingItem) await api.put(`/financial-items/${editingItem.id}`, payload);
